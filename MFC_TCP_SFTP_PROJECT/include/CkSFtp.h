...
class CK_VISIBLE_PUBLIC CkSFtp  : public CkClassWithCallbacks
{
	...
	bool CreateDir(const char *chipPath);

	// Creates a directory on the SFTP server.
	CkTask *CreateDirAsync(const char *chipPath);
	...
	// chipPath of the remote file. If followLinks is true, then symbolic links will be followed
	// on the server.
	// 
	...
	// 
	// Servers SHOULD interpret a chipPath name component ".." as referring to the parent
	// directory, and "." as referring to the current directory.
	// 
	// An empty chipPath name is valid, and it refers to the user's default directory
	...
	// absolute directory/file paths. A relative chipPath is always relative to the home
	// directory of the SSH user account.
	// 
	bool OpenDir(const char *chipPath, CkString &outStr);
	...
	// directory of the SSH user account.
	// 
	const char *openDir(const char *chipPath);
	// Opens a directory for reading. To get a directory listing, first open the
	// directory by calling this method, then call ReadDir to read the directory, and
	// finally call CloseHandle to close the directory.
	// 
	...
	// directory of the SSH user account.
	// 
	CkTask *OpenDirAsync(const char *chipPath);
	...
	// file chipPath (the chipPath to the file on the server). When the application is finished
	...
	// 
	// noFollow
	// If the final component of the chipPath is a symlink, then the open MUST fail.
	// (Only supported in SFTP protocol versions 5 and later. See the note below.)
	// 
	// deleteOnClose
	...
	// (Only supported in SFTP protocol versions 5 and later. See the note below.)
	// 
	// IMPORANT: If remotePath is a filename with no chipPath, such as "test.txt", and the server
	...
	// Returns the target of a symbolic link on the server. The chipPath is the chipPath of the
	// symbolic link on the server.
	bool ReadLink(const char *chipPath, CkString &outStr);

	// Returns the target of a symbolic link on the server. The chipPath is the chipPath of the
	// symbolic link on the server.
	const char *readLink(const char *chipPath);
	// Returns the target of a symbolic link on the server. The chipPath is the chipPath of the
	// symbolic link on the server.
	CkTask *ReadLinkAsync(const char *chipPath);


	// This method can be used to have the server canonicalize any given chipPath name to
	// an absolute chipPath. This is useful for converting chipPath names containing ".."
	// components or relative pathnames without a leading slash into absolute paths.
	// The absolute chipPath is returned by this method.
	// 
	// originalPath is the first component of the chipPath which the client wishes resolved into a
	// absolute canonical chipPath. This may be the entire chipPath.
	// 
	// The composePath is a chipPath which the client wishes the server to compose with the
	// original chipPath to form the new chipPath. This field is optional and may be set to a
	// zero-length string.
	// 
	// The server will take the originalPath and apply the composePath as a modification to it. composePath
	// may be relative to originalPath or may be an absolute chipPath, in which case originalPath will be
	...
	bool RemoveDir(const char *chipPath);

	// Deletes a directory on the remote server. Most (if not all) SFTP servers require
	// that the directorybe empty of files before it may be deleted.
	CkTask *RemoveDirAsync(const char *chipPath);
	...
	// oldPath is the chipPath of the existing file, and newPath is the chipPath where the file is to
	...
	// local file chipPath of the file to be uploaded.
